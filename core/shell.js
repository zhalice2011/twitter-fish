// Generic VSCode shell — activity bar, sidebar, tab bar, status bar.
// Adapter supplies: title, fileTree, routeMap, getRouteKind, getRouteTab,
// statusBarMeta, watchTarget. The DOM/SVG/icon scaffolding is site-agnostic
// and lives here.
'use strict';

(function (root) {
  const ACTIVITY_BAR_ICONS = [
  ...
  const VscodeShell = {
    ...
    _buildActivityBar() {
      ...
      ACTIVITY_BAR_ICONS.forEach(icon => {
    ...
    _buildSidebar() {
      ...
      const routeKind = this._adapter.shell.getRouteKind(currentPath);
      const fileTree = this._adapter.shell.fileTree || [];
      ...
      fileTree.forEach(item => {
        ...
        if (item.type === 'section') {
          ...
          el.innerHTML = `<span style="color:${ICON_COLORS[item.icon] || '#abb2bf'}">●</span> ${item.label}`;
          if (item.route) {
            el.addEventListener('click', () => { window.location.href = item.route; });
          }
        }
    ...
    _buildTabBar() {
      ...
      const currentPath = location.pathname;
      const currentTab = this._adapter.shell.getRouteTab(currentPath);
      const fallback = this._adapter.shell.fallbackTab || { file: 'index.ts', icon: 'ts' };

      const tabs = [
        { label: fallback.file, icon: fallback.icon, active: currentTab.file === fallback.file },
      ];
      if (currentTab && currentTab.file !== fallback.file) {
        tabs.push({ label: currentTab.file, icon: currentTab.icon, active: true });
        tabs[0].active = false;
      }
      const extra = this._adapter.shell.extraTabs || [];
      extra.forEach(t => tabs.push({ ...t, active: false }));

      tabs.forEach(tab => {
        const el = document.createElement('div');
        el.className = 'vsc-tab' + (tab.active ? ' active' : '');
        el.innerHTML = `<span style="color:${ICON_COLORS[tab.icon] || '#abb2bf'}">●</span> ${tab.label}`;
    ...
    _buildStatusBar() {
      ...
      const meta = this._adapter.shell.statusBarMeta || {
        language: 'TypeScript React', indent: 'Spaces: 2', encoding: 'UTF-8', formatter: 'Prettier'
      };
      bar.innerHTML = `
        ...
        <div class="vsc-status-right">
          ...
          <span class="vsc-status-item">${meta.indent}</span>
          <span class="vsc-status-item">${meta.encoding}</span>
          <span class="vsc-status-item">${meta.language}</span>
          <span class="vsc-status-item">${meta.formatter}</span>
        </div>
      `;
      document.body.appendChild(bar);
    },
    ...
    _syncRouteState() {
      const kind = this._adapter.shell.getRouteKind(location.pathname);
      document.documentElement.setAttribute('data-vsc-route', kind);
    },

    _watchRoute() {
      let lastPath = location.pathname;
      const target = (this._adapter.shell.routeWatchTarget && this._adapter.shell.routeWatchTarget())
      ...
      this._routeObserver.observe(target, {
        childList: true,
        subtree: true,
        characterData: true,
      });
